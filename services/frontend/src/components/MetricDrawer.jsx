...
// Métadonnées par métrique : formule + source + unité
const METRIC_META = {
  p_it_kw: {
  ...
  cpu_temp_c: {
    ...
    source:   'Modèle thermique 1er ordre — τ_cpu selon techno (AC: 60s, IC: 30s)',
    unite:    '°C',
  ...
  gpu_temp_c: {
    ...
    source:   'Modèle thermique 1er ordre — τ_gpu selon techno (AC: 120s, IC: 60s)',
    unite:    '°C',
...
export default function MetricDrawer({ metric, history, onClose }) {
  // Fermeture du drawer à l'appui sur Échap
  useEffect(() => {
  ...
  // Récup les métadonnées et dimensions du graphique
  const meta    = METRIC_META[metric.field] || {}
  const rackIds = Object.keys(history)
  const maxLen  = Math.max(...rackIds.map(id => history[id]?.[metric.field]?.length || 0), 0)

  // Génère les labels temporels (ex: "-2s", "-1m", etc)
  ...
  // Prépare les datasets pour le graphique (un par rack)
  const datasets = rackIds.map(rackId => {
    const values = history[rackId]?.[metric.field] || []
    const padded = Array(maxLen - values.length).fill(null).concat(values)
    const techno = rackId.includes('AC') ? 'AC'
                 : rackId.includes('RDHx') ? 'RDHx'
                 : rackId.includes('DLC') ? 'DLC' : 'IC'
  ...
  const options = {
    ...
    plugins: {
      legend: { labels: { color: '#4a5568', font: { family: 'DM Sans', size: 12 }, boxWidth: 12, padding: 16 } },
      tooltip: {
        mode: 'index', intersect: false,
        callbacks: { label: ctx => `${ctx.dataset.label}: ${ctx.parsed.y != null ? f(ctx.parsed.y, 2) : '—'} ${meta.unite || ''}` }
      }
    },
    scales: {
      x: { ticks: { color: '#8898aa', font: { family: 'DM Sans', size: 10 }, maxTicksLimit: 8 }, grid: { color: 'rgba(0,0,0,0.04)' } },
      y: {
        min: 0,
  ...
  // Calcule les stats (min, max, avg) par rack
  const stats = rackIds.map(rackId => {
    const values = history[rackId]?.[metric.field] || []
    const techno = rackId.includes('AC') ? 'AC' : rackId.includes('RDHx') ? 'RDHx' : rackId.includes('DLC') ? 'DLC' : 'IC'
    return {
      rackId, techno,
  ...
  return (
    <>
      ...
      <div onClick={onClose} style={{
        ...
        background: 'rgba(0,0,0,0.2)', zIndex: 200,
        animation: 'fadeOverlay .2s ease',
      }}/>

      {/* Drawer — paneau latéral desktop, feuille de bas mobile */}
      <div className="drawer-panel">
        ...
        <div style={{
          ...
          <div>
            <div style={{ fontSize: '15px', fontWeight: 700, color: 'var(--text)' }}>
              {metric.label}
              {meta.unite && <span style={{ fontSize: '12px', color: 'var(--text3)', fontWeight: 400, marginLeft: '6px' }}>{meta.unite}</span>}
            </div>
        ...
        {/* Cartes stats (dernier, min, max, moyenne par rack) */}
        <div style={{
          ...
          {stats.map(s => (
            <div key={s.rackId} style={{ background: 'var(--surface2)', padding: '12px 16px' }}>
              <div style={{ fontSize: '10px', fontWeight: 600, color: TC[s.techno] || 'var(--cisco)', marginBottom: '6px', textTransform: 'uppercase', letterSpacing: '.06em' }}>
                {s.techno}
              </div>
              <div style={{ fontSize: '18px', fontWeight: 700, color: 'var(--text)', marginBottom: '4px' }}>
                {s.last != null ? f(s.last, 2) : '—'}
                <span style={{ fontSize: '11px', color: 'var(--text3)', fontWeight: 400, marginLeft: '4px' }}>{meta.unite}</span>
              </div>
              <div style={{ fontSize: '10px', color: 'var(--text3)', display: 'flex', gap: '8px' }}>
        ...
        {/* Section formule, source et note technique */}
        {meta.formule && (
          <div style={{
            padding: '14px 20px',
            ...
            <div style={{ fontSize: '10px', fontWeight: 600, color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '.08em', marginBottom: '8px' }}>
              Formule & source
            </div>
            <div style={{
