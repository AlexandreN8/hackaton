...
export default function SimPage({ referentiel, results, loading, onCalculate, aiText, aiStreaming }) {
  ...
  const handleCalc = () => {
    onCalculate({ p_it_kw: parseFloat(pIt), technos: technos.split(','), prix_kwh: 0.15 })
  }
  ...
  return (
    <>
      ...
      <div className="card fade-in">
        <div className="sec-hd" style={{ marginBottom: '14px' }}>Paramètres de simulation</div>
        <div className="cfg-grid">
          <div className="field">
            <label>Charge IT (kW)</label>
            <input type="number" value={pIt} min={1} max={5000} onChange={e => setPIt(e.target.value)} />
          </div>
          <div className="field">
            <label>Mix électrique</label>
            <select value={activeMix} onChange={e => setMix(e.target.value)}>
              {mixList.map(m => (
                <option key={m.scenario} value={m.scenario}>{m.scenario} ({m.co2_kwh} gCO₂/kWh)</option>
              ))}
            </select>
          </div>
          <div className="field">
            <label>Technologies</label>
            <select value={technos} onChange={e => setTechnos(e.target.value)}>
      ...
      {hasResults && (
        <>
          ...
          {/* Hypotheses */}
          <div className="card fade-in">
            <div className="sec-hd" style={{ marginBottom: '14px' }}>Périmètre & hypothèses</div>
            <div className="hyp-grid">
              {filtered.map(r => (
                <div key={r.techno} className="hyp-item">
                  <div className="hyp-title" style={{ color: TC[r.techno] || 'var(--cisco)' }}>
                    {r.techno}
                  </div>
                  <div className="hyp-body">
                    <strong>Périmètre :</strong> {r.e_totale?.perimetre_inclus || '—'}<br />
                    <strong>Exclu :</strong> {r.e_totale?.perimetre_exclus || '—'}<br />
                    <strong>Hypothèse :</strong> {r.co2e_annuel?.hypothese || '—'}<br />
          ...
          <AIReco text={aiText} streaming={aiStreaming} />
        </>
      )}
    </>
  )
