...
const ICONS = {
  env: (
    <svg
      ...
      strokeWidth="2"
    >
      <path d="M12 22V12M12 12C12 7 17 3 22 3c0 5-3 9-10 9zM12 12C12 7 7 3 2 3c0 5 3 9 10 9z" />
    </svg>
  ),
  eco: (
    <svg
  ...
  dry: (
    <svg
      ...
      strokeWidth="2"
    >
      <path d="M12 2v6m0 0C9 12 5 15 5 18a7 7 0 0014 0c0-3-4-6-7-10z" />
    </svg>
  ),
  dense: (
    <svg
...
const PROFILES = [
  {
    ...
    description:
      "Priorité réduction CO₂e et eau — objectifs ESG / GHG Protocol",
  ...
  },
  {
    ...
    label: "Zone sèche",
    description:
      "Contrainte hydrique forte — Moyen-Orient, zones de stress eau",
...
const CRITERIA_DEF = [
  ...
  },
  {
    ...
    tooltip: {
      ...
      source:
        "NVIDIA DGX H100 specs / TIA-942 Telecommunications Infrastructure Standard",
    },
  },
  {
    key: "roi",
...
export default function ScoreChart({ results }) {
  ...
  const rawWeights =
    activeProfile === "custom" ? customWeights : profile.weights;
  ...
  const criteria = CRITERIA_DEF.map((c) => ({
    ...c,
    wNorm: normWeights[c.key],
  }));
  ...
  const options = {
    ...
    scales: {
      x: {
        ...
        min: 0,
        max: 100,
        ticks: { ...CHART_BASE.scales.x.ticks, callback: (v) => `${v}` },
      },
        ...
        {PROFILES.map((p) => (
          <button
            ...
            style={{
              ...
              border: "1px solid",
              borderColor:
                activeProfile === p.id ? "var(--cisco)" : "var(--border)",
              borderRadius: "20px",
              cursor: "pointer",
              fontSize: "12px",
